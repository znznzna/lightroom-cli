...
このツールは **エージェントファースト CLI** として設計されています。AIエージェントが SKILL ファイルを読み、CLIコマンドを呼び出して Lightroom を操作します。以下の3ステップすべてが必要です。
...
#### Step 2: Lightroom プラグインのインストール

Lua プラグインが Lightroom Classic と CLI の間の通信を担います。

```bash
lr plugin install
```

インストール後、Lightroom Classic を再起動してください。**File > Plug-in Manager** に「Lightroom CLI Bridge」が表示されます。

#### Step 3: スキルのインストール（必須）

SKILL ファイルは、AIエージェントが全107コマンドを発見・使用するためのリファレンスです。**これがないとエージェントは CLI を操作できません。** このCLIは人間が直接コマンドを打つのではなく、エージェントが SKILL ファイルを読んで操作する前提で設計されているため、このステップは必須です。
...
Claude Code Plugin がインストールされ、`SKILL.md`（エージェント用の全コマンド・パラメータ・ワークフローのリファレンス）が利用可能になります。

#### コントリビューター向け

> **一般ユーザーはこのセクションをスキップしてください。** 上記の Step 1〜3 だけで利用できます。

lightroom-cli 自体の開発に参加する場合は、pip の代わりにソースからインストールしてください：
...
テストやリントについては [Development](#development) セクションを参照してください。
