...
<!-- CODEX REVIEW FIX: M2 — ソースコード文字列検査を振る舞いテストに変更 -->
...
def test_get_port_file_platform_independent():
    """プラットフォームを変えても同じ結果が返ること（振る舞いテスト）"""
    from lightroom_sdk.paths import get_port_file
    import os
    ...
    with patch.dict("os.environ", {}, clear=True):
        ...
        with patch("sys.platform", "darwin"):
            result_mac = get_port_file()

        # Windows の場合（tempfile.gettempdir() が同じなら同じ結果）
        with patch("sys.platform", "win32"):
            result_win = get_port_file()

        # どちらのプラットフォームでも tempfile.gettempdir() ベースであること
        assert result_mac == Path(tempfile.gettempdir()) / "lightroom_ports.txt"
        assert result_win == Path(tempfile.gettempdir()) / "lightroom_ports.txt"
```

**Step 2: 失敗を確認**
Run: `cd /Users/motokiendo/dev/lightroom-cli && venv/bin/python -m pytest tests/test_paths.py -v -k "test_get_port_file_platform_independent or test_get_port_file_uses_tempfile"`
...
<!-- CODEX REVIEW FIX: I5 — 再接続・ハートビートの回帰テストを追加 -->
...
<!-- CODEX REVIEW FIX: I2 — lightroom://status リソースを追加 -->
...
git commit -m "feat: mcp_server パッケージ骨格を作成 (lightroom://status リソース含む)"
```

### Task 7: tool_registry.py - スキーマ駆動の MCP ツール自動生成

<!-- CODEX REVIEW FIX: C3 — required/min/max を JSON Schema に反映 -->
...
class TestBuildParamField:
    """C3: required/min/max が Field() に正しく反映されること"""

    def test_required_param_no_default(self):
        """required=True のパラメータは default なし"""
        param = ParamSchema("photoId", ParamType.STRING, "Photo ID", required=True)
        annotation, field = build_param_field(param)
        assert annotation is str
        # Field に default が設定されていない（... = required）
        assert field.default is ...

    def test_optional_param_has_default(self):
        """required=False のパラメータは default=None"""
        param = ParamSchema("limit", ParamType.INTEGER, "Limit", required=False)
        annotation, field = build_param_field(param)
        assert field.default is None

    def test_float_with_min_max(self):
        """min/max が Field(ge=, le=) に変換されること"""
        param = ParamSchema("rating", ParamType.FLOAT, "Rating", required=True, min_value=0.0, max_value=5.0)
        annotation, field = build_param_field(param)
        assert annotation is float
        assert field.metadata.get("ge") == 0.0 or field.ge == 0.0
        assert field.metadata.get("le") == 5.0 or field.le == 5.0

    def test_integer_with_min_max(self):
        """integer の min/max"""
        param = ParamSchema("rating", ParamType.INTEGER, "Rating", required=True, min_value=0, max_value=5)
        annotation, field = build_param_field(param)
    ...
    def test_enum_param(self):
        ...
        param = ParamSchema("label", ParamType.ENUM, "Color label", required=True, enum_values=["red", "green", "blue"])
        annotation, field = build_param_field(param)
        # Literal["red", "green", "blue"] 相当
    ...
    def test_description_is_set(self):
        ...
        param = ParamSchema("photoId", ParamType.STRING, "The photo identifier", required=True)
...
class TestCreateToolFunction:
    ...
    def test_function_has_annotations_for_params(self):
        ...
        schema = CommandSchema(
            "catalog.setRating", "catalog.set-rating", "Set rating",
            mutating=True,
            params=[
                ParamSchema("photoId", ParamType.STRING, "Photo ID", required=True),
                ParamSchema("rating", ParamType.INTEGER, "Rating 0-5", required=True, min_value=0, max_value=5),
            ],
        )
        fn = create_tool_function(schema, connection=None)
        # 関数のアノテーションにパラメータが含まれること
...
# C3: ParamSchema から型アノテーション + Field() を生成
_PARAM_TYPE_MAP = {
...
def build_param_field(param: ParamSchema) -> tuple[type, FieldInfo]:
    ...
    - required=True → default=... (必須)
    - required=False → default=None (オプショナル)
    - min/max → Field(ge=, le=)
    - ENUM → Literal[...] アノテーション
    """
    # 型アノテーション
    if param.param_type == ParamType.ENUM and param.enum_values:
        annotation = Literal[tuple(param.enum_values)]
    else:
        annotation = _PARAM_TYPE_MAP.get(param.param_type, str)
...
def create_tool_function(
    ...
    C3: パラメータごとに型アノテーション + Field() を使って required/min/max を反映。
    """
    tool_name = sanitize_tool_name(schema.command)

    # パラメータのアノテーションと Field を構築
    ...
    # 型アノテーションを関数に設定（fastmcp が inspect で読む）
    tool_fn.__annotations__ = param_annotations.copy()
    # Field デフォルトを __defaults__ 相当に設定
    # fastmcp は inspect.signature を使うため、__kwdefaults__ に設定
    if param_defaults:
        tool_fn.__kwdefaults__ = {
            name: field.default for name, field in param_defaults.items()
...
git commit -m "feat: tool_registry.py - スキーマ駆動の MCP ツール自動生成 (required/min/max 反映)"
```
...
<!-- CODEX REVIEW FIX: C1 — mutating 再送禁止を実装 -->
<!-- CODEX REVIEW FIX: C2 — client._bridge 直叩きを廃止、LightroomClient.execute_command() 経由に変更 -->
<!-- CODEX REVIEW FIX: M1 — Lock直列化テストを改善（タイムスタンプで直列実行を証明） -->
...
async def test_lazy_connect(mock_lr_server):
    ...
    assert result["result"]["status"] == "ok" or result.get("status") == "ok"
...
async def test_mutating_not_retried_after_reconnect():
    ...
    assert result["isError"] is True
    # mutating コマンドは MUTATING_NOT_RETRIED コードを返すこと
    assert result["code"] in ("CONNECTION_ERROR", "MUTATING_NOT_RETRIED")
    await cm.shutdown()
...
async def test_mutating_flag_prevents_retry_explicitly(mock_lr_server):
    ...
    # クライアントを強制切断して再接続が必要な状態にする
    cm._client = None  # 接続を失った状態をシミュレート
    cm._reconnected = True  # 再接続フラグを立てる
    ...
    assert "再送" in result["message"] or "retry" in result["message"].lower()
    await cm.shutdown()
...
async def test_readonly_command_retried_after_reconnect(mock_lr_server):
    ...
    # 接続して切断状態をシミュレート
    await cm.execute("system.ping", {}, timeout=5.0, mutating=False)
    cm._client = None  # 接続を失った状態
    cm._reconnected = True

    # read-only コマンドは再接続して成功する
    result = await cm.execute("system.ping", {}, timeout=5.0, mutating=False)
    # 再接続に成功した場合は結果が返る、失敗した場合は CONNECTION_ERROR
    # （mock_lr_server は生きているので成功するはず）
...
class ConnectionManager:
    ...
    設計原則:
    - コマンド実行は LightroomClient.execute_command() 経由（C2）
    - バリデーションは ConnectionManager 側で事前実行
    - mutating コマンドは再接続後に再送しない（C1）
    """
    ...
    def __init__(self, port_file: str | None = None):
        ...
        self._reconnected = False  # C1: 再接続が発生したかどうか
    ...
    async def execute(
        ...
        C1: mutating=True の場合、再接続が発生していたら再送せずエラー返却。
        C2: LightroomClient.execute_command() を経由してコマンド実行。
        """
        # 1. Validation（ConnectionManager 側で事前実行）
        from lightroom_sdk.validation import ValidationError, validate_params
        ...
        # 3. Execute with lock (C2: execute_command 経由)
        try:
            ...
            self._reconnected = True  # C1: 再接続フラグ
            if mutating:
                return {
                    "isError": True,
                    "code": "MUTATING_NOT_RETRIED",
...
git commit -m "feat: ConnectionManager - lazy connect, asyncio.Lock, mutating再送禁止, execute_command経由"
```
...
<!-- CODEX REVIEW FIX: I1 — lr mcp test を実際の ping 接続確認に変更 -->
<!-- CODEX REVIEW FIX: I4 — lr mcp install に --force フラグと既存エントリ上書き確認を追加 -->
...
class TestMcpInstall:
    ...
    def test_install_warns_on_existing_entry_without_force(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "mcpServers": {"lightroom-cli": original_entry}
        }))
        ...
        with patch("cli.commands.mcp._get_claude_config_path", return_value=config_file):
            ...
            # --force なしなので上書きされない
            assert result.exit_code == 0
            assert "already installed" in result.output.lower() or "既にインストール" in result.output
    ...
    def test_install_overwrites_with_force(self, tmp_path):
        ...
        config_file = tmp_path / "claude_desktop_config.json"
        config_file.write_text(json.dumps({
            "mcpServers": {"lightroom-cli": {"command": "old-lr-mcp"}}
        }))
...
class TestMcpTest:
    # I1: lr mcp test が実際に ping を送信すること
    def test_mcp_test_sends_ping(self, mock_lr_server):
        """I1: lr mcp test が実際に Lightroom に接続して ping を送信する"""
        mock_lr_server.register_response("system.ping", {"status": "ok"})

        with (
            patch("cli.commands.mcp._check_fastmcp_installed", return_value=True),
            patch("cli.commands.mcp._get_port_file_for_test", return_value=str(mock_lr_server.port_file)),
        ):
            runner = CliRunner()
...
# I1: lr mcp test を実際の ping 接続確認に変更
@mcp.command()
def test():
...
git commit -m "feat: lr mcp install/uninstall/status/test コマンド追加 (--force, ping接続確認)"
```
...
<!-- CODEX REVIEW FIX: I3 — fastmcp TestClient 経由のテストに変更 -->
...
from fastmcp import FastMCP
from fastmcp.client import Client as TestClient
...
class TestMcpServerViaTestClient:
    ...
    async def test_ping_via_testclient(self, mcp_app, mock_lr_server):
        ...
        async with TestClient(server) as client:
            ...
            # fastmcp TestClient は結果をラップする場合がある
...
git commit -m "test: MCP Server 統合テスト追加 (fastmcp TestClient 経由)"
```
...
<!-- CODEX REVIEW FIX: M3 — E2E テストスケルトン作成を追加 -->
...
Run: `cd /Users/motokiendo/dev/lightroom-cli && ls tests/e2e/test_mcp_e2e.py`
Expected: ファイルが存在すること

**Step 6: コミット**
```bash
git add tests/e2e/test_mcp_e2e.py
git commit -m "test: MCP E2E テストスケルトン追加"
```
...
| `tests/e2e/test_mcp_e2e.py` | Create | 9 |

## Codex レビュー指摘 反映サマリー

| ID | 重要度 | 内容 | 反映タスク |
|----|--------|------|-----------|
| C1 | Critical | `mutating` 再送禁止が未実装 | Task 8 — `_reconnected` フラグ + `MUTATING_NOT_RETRIED` エラーコード |
| C2 | Critical | `ConnectionManager` が `client._bridge` を直叩き | Task 8 — `client.execute_command()` 経由に変更 |
| C3 | Critical | ツールスキーマの `required/min/max` が未反映 | Task 7 — `build_param_field()` 追加、型アノテーション + `Field()` 生成 |
| I1 | Important | `lr mcp test` が接続確認になっていない | Task 10 — 実際に `connect()` + `ping()` を実行 |
| I2 | Important | MCP Resource `lightroom://status` が計画から脱落 | Task 6 — `_run.py` に `@mcp.resource("lightroom://status")` 追加 |
| I3 | Important | MCP統合テストが TestClient を通していない | Task 11 — `fastmcp.client.Client` (TestClient) 経由のテストに変更 |
| I4 | Important | `lr mcp install` の既存エントリ上書き確認がない | Task 10 — `--force` フラグ追加、既存時は警告表示 |
| I5 | Important | Task 5 の回帰テスト不足 | Task 5 — 再接続テスト + ハートビート設定確認テスト追加 |
| M1 | Minor | Lock直列化テストが不十分 | Task 8 — タイムスタンプで実行区間の非重複を証明 |
| M2 | Minor | `get_port_file` テストにソースコード文字列検査 | Task 2 — 振る舞いテスト（monkeypatch でプラットフォーム変更）に変更 |
| M3 | Minor | E2E テストがタスク化されていない | Task 17 — `tests/e2e/test_mcp_e2e.py` スケルトン作成ステップ追加 |
