...
def test_selection_flag(mock_get_bridge, runner):
    ...
    mock_bridge.send_command.assert_called_once_with("selection.flagAsPick", {}, timeout=30.0)
...
def test_selection_reject(mock_get_bridge, runner):
    ...
    assert result.exit_code == 0
    mock_bridge.send_command.assert_called_once_with("selection.flagAsReject", {}, timeout=30.0)
...
def test_selection_unflag(mock_get_bridge, runner):
    ...
    assert result.exit_code == 0
    mock_bridge.send_command.assert_called_once_with("selection.removeFlag", {}, timeout=30.0)
...
def test_selection_next(mock_get_bridge, runner):
    ...
    assert result.exit_code == 0
    mock_bridge.send_command.assert_called_once_with("selection.nextPhoto", {}, timeout=30.0)
...
def test_selection_previous(mock_get_bridge, runner):
    ...
    assert result.exit_code == 0
    mock_bridge.send_command.assert_called_once_with("selection.previousPhoto", {}, timeout=30.0)
...
def test_selection_color_label(mock_get_bridge, runner):
    ...
    assert result.exit_code == 0
    mock_bridge.send_command.assert_called_once_with("selection.setColorLabel", {"label": "red"}, timeout=30.0)
...
def test_selection_select_inverse(mock_get_bridge, runner):
    ...
    mock_bridge.send_command.return_value = {
        "id": "9",
        "success": True,
        "result": {"message": "Selection inverted"},
    }
    mock_get_bridge.return_value = mock_bridge
...
def test_selection_increase_rating(mock_get_bridge, runner):
    ...
    mock_bridge.send_command.return_value = {
        "id": "10",
        "success": True,
        "result": {"message": "Rating increased"},
    }
    mock_get_bridge.return_value = mock_bridge
...
def test_selection_decrease_rating(mock_get_bridge, runner):
    ...
    mock_bridge.send_command.return_value = {
        "id": "11",
        "success": True,
        "result": {"message": "Rating decreased"},
    }
    mock_get_bridge.return_value = mock_bridge
...
def test_selection_toggle_label_red(mock_get_bridge, runner):
    ...
    mock_bridge.send_command.return_value = {
        "id": "12",
        "success": True,
        "result": {"message": "Red label toggled"},
    }
    mock_get_bridge.return_value = mock_bridge
...
def test_selection_toggle_label_blue(mock_get_bridge, runner):
    ...
    mock_bridge.send_command.return_value = {
        "id": "13",
        "success": True,
        "result": {"message": "Blue label toggled"},
    }
    mock_get_bridge.return_value = mock_bridge
...
def test_selection_extend(mock_get_bridge, runner):
    ...
    mock_bridge.send_command.return_value = {
        "id": "14",
        "success": True,
        "result": {"message": "Selection extended"},
    }
    mock_get_bridge.return_value = mock_bridge
    result = runner.invoke(cli, ["selection", "extend", "--direction", "right", "--amount", "2"])
    assert result.exit_code == 0
    mock_bridge.send_command.assert_called_once_with(
        "selection.extendSelection", {"direction": "right", "amount": 2}, timeout=30.0
    )
...
def test_selection_deselect_others(mock_get_bridge, runner):
    ...
    mock_bridge.send_command.return_value = {
        "id": "16",
        "success": True,
        "result": {"message": "Others deselected"},
    }
    mock_get_bridge.return_value = mock_bridge
...
def test_selection_get_flag(mock_get_bridge, runner):
    ...
    mock_bridge.send_command.return_value = {
        "id": "17",
        "success": True,
        "result": {"pickStatus": 1, "label": "Pick"},
    }
    mock_get_bridge.return_value = mock_bridge
