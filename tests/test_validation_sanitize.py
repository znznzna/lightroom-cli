...
class TestStringSanitization:
    def test_null_character_rejected(self):
        with pytest.raises(ValidationError, match="null character"):
            validate_params("develop.setValue", {"param": "Exp\x00sure", "value": 0.5})

    def test_control_character_rejected(self):
        with pytest.raises(ValidationError, match="control character"):
            validate_params("develop.setValue", {"param": "Exp\x01sure", "value": 0.5})

    def test_bell_character_rejected(self):
        with pytest.raises(ValidationError, match="control character"):
            validate_params("develop.setValue", {"param": "Exp\x07sure", "value": 0.5})

    def test_tab_allowed(self):
        result = validate_params("develop.setValue", {"param": "Exposure\t", "value": 0.5})
        assert "\t" in result["param"]
    ...
    def test_overlong_string_rejected(self):
        ...
        with pytest.raises(ValidationError, match="exceeds maximum length"):
            validate_params("develop.setValue", {"param": long_str, "value": 0.5})

    def test_max_length_string_passes(self):
        exact_str = "a" * 10_000
        result = validate_params("develop.setValue", {"param": exact_str, "value": 0.5})
        assert len(result["param"]) == 10_000
