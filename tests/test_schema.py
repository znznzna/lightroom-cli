...
class TestSchemaIntegrity:
    ...
    def test_enum_params_have_values(self):
        from lightroom_sdk.schema import ParamType, get_all_schemas

        for name, schema in get_all_schemas().items():
            for param in schema.params:
                if param.type == ParamType.ENUM:
                    assert param.enum_values, f"ENUM param '{param.name}' in '{name}' has no enum_values"
    ...
    def test_no_duplicate_cli_paths(self):
        ...
        assert len(non_template) == len(set(non_template)), (
            f"Duplicate cli_paths found: {[p for p in non_template if non_template.count(p) > 1]}"
        )
